...
//        return false;
    ...
    public boolean addChildrenToSolver(LinearSystem system) {
        ...
        if (hasBarriers) {
            for (int i = 0; i < count; i++) {
                ConstraintWidget widget = mChildren.get(i);
                if (widget instanceof CoreBarrier) {
                    ((CoreBarrier) widget).markWidgets();
                }
            }
        }
        ...
        for (int i = 0; i < count; i++) {
            ConstraintWidget widget = mChildren.get(i);
            if (widget.addFirst()) {
                if (widget instanceof CoreVirtualLayout) {
        ...
        while (widgetsToAdd.size() > 0) {
            ...
            CoreVirtualLayout layout = null;
            for (ConstraintWidget widget : widgetsToAdd) {
                layout = (CoreVirtualLayout) widget;
    ...
    public static boolean measure(int level, ConstraintWidget widget, BasicMeasure.Measurer measurer, BasicMeasure.Measure measure, int measureStrategy) {
        ...
        if (widget.getVisibility() == GONE
                || widget instanceof CoreGuideline
                || widget instanceof CoreBarrier) {
            if (DEBUG) {
        ...
        if (pass == 0 && Optimizer.enabled(mOptimizationLevel, Optimizer.OPTIMIZATION_DIRECT)) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (child.isMeasureRequested()
                        && !(child instanceof CoreGuideline)
                        && !(child instanceof CoreBarrier)
                        && !(child instanceof CoreVirtualLayout)
     ...
     */
    public ArrayList<CoreGuideline> getVerticalGuidelines() {
        ArrayList<CoreGuideline> guidelines = new ArrayList<>();
        for (int i = 0, mChildrenSize = mChildren.size(); i < mChildrenSize; i++) {
            ...
            if (widget instanceof CoreGuideline) {
                ...
                if (guideline.getOrientation() == CoreGuideline.VERTICAL) {
     ...
     * @return array of guidelines
     */
    public ArrayList<CoreGuideline> getHorizontalGuidelines() {
        ArrayList<CoreGuideline> guidelines = new ArrayList<>();
        for (int i = 0, mChildrenSize = mChildren.size(); i < mChildrenSize; i++) {
            final ConstraintWidget widget = mChildren.get(i);
            if (widget instanceof CoreGuideline) {
                CoreGuideline guideline = (CoreGuideline) widget;
                if (guideline.getOrientation() == CoreGuideline.HORIZONTAL) {
                    guidelines.add(guideline);
                }
            }
        }
    ...
    public void setPass(int pass) {
        this.pass = pass;}public void release() {reset();mDependencyGraph = null;mBasicMeasureSolver = null;mSystem = null;mMeasurer = null;mVerticalChainsArray = null;mHorizontalChainsArray = null;frame = null;mCenter = null;mLeft = null;mRight = null;mTop = null;mBottom = null;mBaseline = null;mCenterX = null;mCenterY = null;mListAnchors = null;mAnchors = null;
    }
}
