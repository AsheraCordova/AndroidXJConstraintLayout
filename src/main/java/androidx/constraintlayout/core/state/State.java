...
import androidx.constraintlayout.core.state.helpers.GuidelineReferenceHelper;
...
public class State {
    ...
    public GuidelineReferenceHelper horizontalGuideline(Object key) {
        return guideline(key, ConstraintWidget.HORIZONTAL);
    }

    public GuidelineReferenceHelper verticalGuideline(Object key) {
        return guideline(key, ConstraintWidget.VERTICAL);
    }

    public GuidelineReferenceHelper guideline(Object key, int orientation) {
        ConstraintReference reference = constraints(key);
        if (reference.getFacade() == null || !(reference.getFacade() instanceof GuidelineReferenceHelper)) {
            GuidelineReferenceHelper guidelineReference = new GuidelineReferenceHelper(this);
            guidelineReference.setOrientation(orientation);
            guidelineReference.setKey(key);
            reference.setFacade(guidelineReference);
        }
        return (GuidelineReferenceHelper) reference.getFacade();
    }
    ...
    public void apply(ConstraintWidgetContainer container) {
        ...
        for (Object key : mReferences.keySet()) {
            ...
            if (reference != mParent) {
                ...
                if (reference.getFacade() instanceof GuidelineReferenceHelper) {
