...
public class MotionController {
    ...
    private MLMotionPaths mStartMotionPath = new MLMotionPaths();
    private MLMotionPaths mEndMotionPath = new MLMotionPaths();

    private MLMotionConstrainedPoint mStartPoint = new MLMotionConstrainedPoint();
    private MLMotionConstrainedPoint mEndPoint = new MLMotionConstrainedPoint();
    ...
    private ArrayList<MLMotionPaths> mMLMotionPaths = new ArrayList<>();
    ...
    MLMotionPaths getKeyFrame(int i) {
        return mMLMotionPaths.get(i);
    }
    ...
    void buildPath(float[] points, int pointCount) {
        ...
        for (int i = 0; i < pointCount; i++) {
            ...
            float start = 0;
            float end = Float.NaN;
            for (MLMotionPaths frame : mMLMotionPaths) {
    ...
    int buildKeyFrames(float[] keyFrames, int[] mode) {
        if (keyFrames != null) {
            ...
            if (mode != null) {
                for (MLMotionPaths keyFrame : mMLMotionPaths) {
    ...
    private void insertKey(MLMotionPaths point) {
        int pos = Collections.binarySearch(mMLMotionPaths, point);
        if (pos == 0) {
            Log.e(TAG, " KeyPath position \"" + point.position + "\" outside of range");
        }
        mMLMotionPaths.add(-pos - 1, point);
    }
    ...
    public void setup(int parentWidth, int parentHeight, float transitionDuration, long currentTime) {
        ...
        if (DEBUG) {
            if (mKeyList == null) {
                ...
                Log.v(TAG, ">>>>>>>>>>>>>>> mKeyList for " + CLDebug.getName(mView));
        ...
        if (DEBUG) {
            ...
            Log.v(TAG, ">>>>>>>>>>>>>>> MLMotionConstrainedPoint found " + Arrays.toString(attr.toArray()));
        }
        if (mKeyList != null) {
            for (Key key : mKeyList) {
                if (key instanceof KeyPosition) {
                    KeyPosition keyPath = (KeyPosition) key;
                    insertKey(new MLMotionPaths(parentWidth, parentHeight, keyPath, mStartMotionPath, mEndMotionPath));
        ...
        MLMotionPaths[] points = new MLMotionPaths[2 + mMLMotionPaths.size()];
        int count = 1;
        points[0] = mStartMotionPath;
        points[points.length - 1] = mEndMotionPath;
        if (mMLMotionPaths.size() > 0 && mCurveFitType == KeyFrames.UNSET) {
            mCurveFitType = CurveFit.SPLINE;
        }
        for (MLMotionPaths point : mMLMotionPaths) {
            points[count++] = point;
        }
        ...
        for (int j = 0; j < mInterpolateVariables.length; j++) {
            ...
            if (interpolateVariable < MLMotionPaths.names.length) {
                String s = MLMotionPaths.names[mInterpolateVariables[j]] + " [";
                for (int i = 0; i < points.length; i++) {
                    s += splineData[i][j];
                }
            }
    ...
    private void readView(MLMotionPaths motionPaths) {
    ...
    void endTrigger(boolean start) {
        if ("button".equals(CLDebug.getName(mView)))
