...
class TouchResponse {
   ...
   /**
    ...
    void processTouchEvent(MotionEvent event, MotionLayout.MotionTracker velocityTracker, int currentState, MotionScene motionScene) {
        if (DEBUG) {
            Log.v(TAG, CLDebug.getLocation() + " best processTouchEvent For ");
        }
        if (mIsRotateMode) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (Math.abs(drag) > mDragThreshold || mDragStarted) {
                    ...
                    if (mTouchAnchorId != MotionScene.UNSET) {
                        ...
                        mMotionLayout.getAnchorDpDt(mTouchAnchorId, pos, mTouchAnchorX, mTouchAnchorY, mAnchorDpDt);
                        if (DEBUG) {
                            Log.v(TAG, CLDebug.getLocation() + " mAnchorDpDt " + Arrays.toString(mAnchorDpDt));
                        }
                    } else {
                        if (DEBUG) {
                            Log.v(TAG, CLDebug.getLocation() + " NO ANCHOR ");
                        }
   ...
   /**
    ...
    void setupTouch() {
        ...
        if (mTouchAnchorId != -1) {
            ...
            if (view == null) {
                Log.e(TAG, "cannot find TouchAnchorId @id/" + CLDebug.getName(mMotionLayout.getContext(), mTouchAnchorId));
            }
        }
