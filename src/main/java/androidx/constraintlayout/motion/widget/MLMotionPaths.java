 ...
 * @hide
 */
class MLMotionPaths implements Comparable<MLMotionPaths> {
     ...
     * @param endTimePoint
     */
    void initCartesian(KeyPosition c, MLMotionPaths startTimePoint, MLMotionPaths endTimePoint) {
        float position = c.mFramePosition / 100f;
        MLMotionPaths point = this;
        point.time = position;
        ...
        point.mMode = MLMotionPaths.CARTESIAN;
     ...
     * @param endTimePoint
     */
    public MLMotionPaths(int parentWidth, int parentHeight, KeyPosition c, MLMotionPaths startTimePoint, MLMotionPaths endTimePoint) {
    ...
    void initPolar(int parentWidth, int parentHeight, KeyPosition c, MLMotionPaths s, MLMotionPaths e) {
        float position = c.mFramePosition / 100f;
        this.time = position;
        mDrawPath = c.mDrawPath;
    ...
    public void setupRelative(MotionController mc, MLMotionPaths relative) {
    ...
    void initScreen(int parentWidth, int parentHeight, KeyPosition c, MLMotionPaths startTimePoint, MLMotionPaths endTimePoint) {
        float position = c.mFramePosition / 100f;
        MLMotionPaths point = this;
        point.time = position;
        ...
        point.mMode = MLMotionPaths.SCREEN;
    ...
    void initPath(KeyPosition c, MLMotionPaths startTimePoint, MLMotionPaths endTimePoint) {
        ...
        point.mMode = MLMotionPaths.PERPENDICULAR;
        point.x = (int) (startTimePoint.x + (pathVectorX) * path - scaleX * scaleWidth / 2);
        point.y = (int) (startTimePoint.y + (pathVectorY) * path - scaleY * scaleHeight / 2);
        point.x += normalX;
        point.y += normalY;
    ...
    void different(MLMotionPaths points, boolean[] mask, String[] custom, boolean arcMode) {
        int c = 0;
    ...
    void setView(float position, View view, int[] toUse, double[] data, double[] slope, double[] cycle) {
        ...
        for (int i = 0; i < mTempValue.length; i++) {
            ...
            if (DEBUG) {
                Log.v(TAG, CLDebug.getName(view) + " set " + names[i]);
            }
            switch (i) {
                case OFF_POSITION:
                    delta_path = value;
    ...
    @Override
    public int compareTo(@NonNull MLMotionPaths o) {
